...
export default function CreateStore() {
    ...
    const fetchSellerStatus = async () => {
        try {
            ...
            if (!res.ok) {
                throw new Error(data.error || "Failed to check store status")
            }
    ...
    const onSubmitHandler = async (e) => {
        ...
        setAlreadySubmitted(true)
        setStatus("pending")
        setMessage("Your store application has been submitted for review.")
    }
        ...
        <>
            {!alreadySubmitted ? (
                <div className="mx-6 min-h-[70vh] my-16">
                    <form onSubmit={e => toast.promise(onSubmitHandler(e), { loading: "Submitting data...", success: "Store submitted!", error: (err) => err.message })} className="max-w-7xl mx-auto flex flex-col items-start gap-3 text-slate-500">
                        <div>
                        ...
                        <p>Name</p>
                        <input name="name" onChange={onChangeHandler} value={storeInfo.name} type="text" placeholder="Enter your store name" required className="border border-slate-300 outline-slate-400 w-full max-w-lg p-2 rounded" />

                        <p>Description</p>
                        <textarea name="description" onChange={onChangeHandler} value={storeInfo.description} rows={5} placeholder="Enter your store description" required className="border border-slate-300 outline-slate-400 w-full max-w-lg p-2 rounded resize-none" />

                        <p>Email</p>
                        <input name="email" onChange={onChangeHandler} value={storeInfo.email} type="email" placeholder="Enter your store email" required className="border border-slate-300 outline-slate-400 w-full max-w-lg p-2 rounded" />

                        <p>Contact Number</p>
                        <input name="contact" onChange={onChangeHandler} value={storeInfo.contact} type="text" placeholder="Enter your store contact number" required className="border border-slate-300 outline-slate-400 w-full max-w-lg p-2 rounded" />
